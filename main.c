...
void fenetre(MaFenetre *wind)
{
        ...
        gtk_window_set_default_size(GTK_WINDOW(wind->wind), 700, 700);
        gtk_window_set_position(GTK_WINDOW(wind->wind), GTK_WIN_POS_CENTER);
        gtk_window_set_resizable(GTK_WINDOW(wind->wind), FALSE);
}
...
void Connect_to_headerbar(Headerbar *hbar, GtkWidget *widget, gboolean start)
{
        if (start)
                gtk_header_bar_pack_start(GTK_HEADER_BAR(hbar->hbar), widget);
        else
                gtk_header_bar_pack_end(GTK_HEADER_BAR(hbar->hbar), widget);
}
...
void Connect_to_Box(GtkWidget *box, GtkWidget *widget, gboolean start)
{
        if (start)
...
int myboard[8][8] = {        // Tableau représentant l'état du jeu avec les pièces
    {EMPTY, WHITE, EMPTY, BLACK, EMPTY, BLACK, EMPTY, BLACK},
...
static void onAccept(dyad_Event *e)
{
        ...
        dyad_writef(e->remote, "echo server Jalal\r\n");
        lokman = e->remote;
}
...
static void field_jump(View *view, Field *field[8][8], Field *piece)
{
        ...
        /* Déplacer les pièces */
        PlaySound(TEXT("C:\\Users\\HP\\Desktop\\gtk\\test\\move_sound_effect.wav"), NULL, SND_FILENAME | SND_ASYNC);
...
static void new_game_button_clicked(GtkWidget *button, View *view)
{
        ...
        if (shaders == 1)
        {
                ...
                Connect_to_Box(view->box, view->grid, TRUE);
...
void *gtkThread(gint argc, gchar *argv[])
{
        ...
        prSimpleBut *newbutton = init_button(newbutton, "New");
        create_button(newbutton);

        // Initialisation du bouton "Dernier mouvement"
        prSimpleBut *lastmove = init_button(lastmove, "Last");
        create_button(lastmove);

        // Initialisation du bouton "Prochain mouvement"
        prSimpleBut *nextmove = init_button(nextmove, "Next");
        ...
        add_item_combobox(mode, "H vs H");
        add_item_combobox(mode, "Ai vs H");
        ...
        add_item_combobox(color, "W");
        add_item_combobox(color, "B");
        ...
        Connect_to_headerbar(hbar, newbutton->button, FALSE);
        Connect_to_headerbar(hbar, lastmove->button, FALSE);
        Connect_to_headerbar(hbar, nextmove->button, FALSE);
        Connect_to_headerbar(hbar, difficulty->combo, TRUE);
        Connect_to_headerbar(hbar, mode->combo, TRUE);
        Connect_to_headerbar(hbar, color->combo, TRUE);
        ...
        Connect_to_Box(box->box, Shader_Create(&Data, win->wind), TRUE);
...
void *dyadThread(void *arg)
{
        ...
        dyad_listenEx(server, "192.168.31.67", 9999, 1);

        printf("Waiting for peer to connect...\n");
        dyad_connect(client, "192.168.31.21", 8888);
